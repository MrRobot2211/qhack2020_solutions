...
def natural_gradient(params):
    ...
    state_val = variational_state(params) 
    
    def get_sq_corr(phi):
        
        prod = np.vdot(state_val,phi)
        
        return prod * np.conj(prod)
    
    for i in range(6):
        
        unit_vectori = np.zeros_like(params)
        unit_vectori[i] += 1
        
      
        
        F[i,i] = (2 -get_sq_corr(variational_state(params + 2*epsilon * unit_vectori )) -  get_sq_corr(variational_state(params - 2*epsilon * unit_vectori )) )/8
        
        for j in range(i):
            
            
            unit_vectorj = np.zeros_like(params)
            unit_vectorj[j] += 1

            
            F[i,j] = (- get_sq_corr(variational_state(params + epsilon * (unit_vectori + unit_vectorj))) 
                 +get_sq_corr(variational_state(params - epsilon * (unit_vectori - unit_vectorj)))
                    + get_sq_corr(variational_state(params - epsilon * (unit_vectorj - unit_vectori)))
                             - get_sq_corr(variational_state(params - epsilon * (unit_vectori + unit_vectorj)))
             )/8
            #print(F[i,j])
    ...
    for i in range(6):
        ...
        gradient[i] =  (qnode(params + epsilon * unit_vectori).numpy() - qnode(params - epsilon * unit_vectori).numpy())/2
    
    natural_grad = np.dot(np.linalg.inv(F.numpy()),gradient).numpy()
